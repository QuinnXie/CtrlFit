...
def _make_ctrlfit_options(
    ...
    """Merge ctrlfit signature defaults, options, and explicit overrides."""
    ...
    return _make_options(options, **option_data)
...
def _score_candidate_model_closed_loop(
    ...
    *,
    ...
    for index, (x_i, y_i, y_ref_i) in enumerate(zip(x_items, y_items, y_ref_items)):
        ...
        y_pred_i = _as_2d(results["Y_true"])
...
def ctrlfit(
    ...
    **option_overrides: Any,
    ...
    n_jobs: int or None
        ...
        behavior, which sets n_jobs to cpu_count().
    ...
    num_lyap_steps: int
        Lyapunov descent horizon. Only 2 is currently supported. Other values
        emit a warning and are replaced with 2.
    lyap_mu: float
    ...
    y_mean, y_gain, u_mean, u_gain: array-like, scalar, or None
        Scaling statistics. Defaults are identity scaling (0 mean, 1 gain).
        The output statistics are reused for the reference part of [Y, Y_ref].
        Pass a mean/gain pair as None with use_scaling=True to estimate that
        pair from the training data.
    ...
    if opt.n_jobs is not None and int(opt.n_jobs) < 1:
        raise ValueError("n_jobs must be a positive integer or None")
    if opt.num_lyap_steps != 2:
        warnings.warn(
            f"num_lyap_steps={opt.num_lyap_steps} is not supported; "
            "ctrlfit currently supports only num_lyap_steps=2. Using 2.",
            UserWarning,
            stacklevel=2,
        )
        opt.num_lyap_steps = 2
    if opt.enable_x64 and not jax.config.jax_enable_x64:  # type: ignore
        jax.config.update("jax_enable_x64", True)

    if opt.use_lyap_reg and (state_fcn is None or output_fcn is None):
        raise ValueError("state_fcn and output_fcn are required when use_lyap_reg=True")
    elif opt.use_lyap_reg and X_hat is None:
    ...
    if has_validation_data and (U_val is None or Y_val is None or Y_ref_val is None):
        raise ValueError("Validation requires U_val, Y_val, and Y_ref_val together")
    if U_val is not None:
        # Validation data reuse the training scalers so candidate selection is
        # performed in the same coordinate system as training.
        ny = problem.ny
    ...
    if opt.use_parallel_fit and validation_problem is not None and opt.closed_loop_validation:
        ...
        if missing_closed_loop_inputs:
            ...
            use_closed_loop_validation = True
    ...
    if opt.use_lyap_reg and X_hat is None and custom_reg_fcn is None:
        raise ValueError(
            "Default Lyapunov regularization requires `X_hat` "
            "(state-estimate trajectories). Provide `X_hat`, pass "
            "`custom_reg_fcn`, or set use_lyap_reg=False."
        )
    if opt.use_lyap_reg and X_hat is None and init_params_fcn is not None and init_lyap_fcn is None:
        raise ValueError(
            "Default Lyapunov initialization requires `X_hat` to infer the "
            "plant-state dimension. Provide `X_hat`, pass `init_lyap_fcn`, "
            "or set use_lyap_reg=False."
        )
    ...
    if init_params_fcn is not None:
        def full_init_fcn(seed):
            params = list(init_params_fcn(seed))
            if opt.use_lyap_reg:
                # The Lyapunov tail is always appended at the end. This keeps
                # unpacking independent of the number of neural-network params.
                if init_lyap_fcn is not None:
                    ...
                    lyap_params = initialize_lyapunov_tail(seed + 10000, nx_physical, int(model.nx))
                params = params + lyap_params
            return params
    ...
    if opt.use_lyap_reg and len(model.params) < 6:
        raise ValueError(
            "use_lyap_reg=True requires model parameters ending with "
            "[z_eq, psi_L, psi_Q, eta_L_raw, eta_Q_raw, tau]."
        )
    params_min = _make_tau_nonnegative_params_min(model.params) if opt.use_lyap_reg else None
    # params_min constrains only the final tau entry when Lyapunov training is
    # enabled; the remaining parameters stay unbounded.
    model.optimization(
    ...
    if opt.verbose:
        ...
        if opt.use_lyap_reg:
            lyap_msg += f" (m={opt.num_lyap_steps})"
        print(f"{lyap_msg}\n")
