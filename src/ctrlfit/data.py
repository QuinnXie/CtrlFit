...
def _compute_scaler(
    ...
    if not enabled:
        if mean is not None and gain is not None:
            mean_arr = _as_scaling_vector(mean, dim, "mean")
            gain_arr = _as_scaling_vector(gain, dim, "gain")
            _validate_scaler_values(mean_arr, gain_arr, mean_name="mean", gain_name="gain")
            if not (np.array_equal(mean_arr, np.zeros(dim)) and np.array_equal(gain_arr, np.ones(dim))):
                raise ValueError(
                    "Nonidentity mean/gain cannot be used when use_scaling=False; "
                    "pre-scale all user data and callbacks instead"
                )
        return _identity_scaler(dim)
    if mean is None and gain is None:
        raise ValueError("use_scaling=True requires explicit mean and gain values")
    mean_arr = _as_scaling_vector(mean, dim, "mean")
    gain_arr = _as_scaling_vector(gain, dim, "gain")
    _validate_scaler_values(mean_arr, gain_arr, mean_name="mean", gain_name="gain")
...
def _validate_scaler_values(
    ...
    *,
    ...
    if not np.all(np.isfinite(gain)):
        raise ValueError(f"{gain_name} must contain only finite values")
    if np.any(gain <= 0.0):
        raise ValueError(f"{gain_name} must be strictly positive")


def scale_control_bounds(
    u_min: Any,
    u_max: Any,
    *,
    u_mean: Any,
    ...
    """Transform plant-coordinate control bounds into model-output coordinates.

    The affine convention is ``u_scaled = (u - u_mean) * u_gain``. Gains must
    be finite and strictly positive so lower and upper bounds preserve order.
    """

    sizes = [np.asarray(value).size for value in (u_min, u_max, u_mean, u_gain)]
    dim = int(max(sizes))
    u_min_arr = _as_scaling_vector(u_min, dim, "u_min")
    u_max_arr = _as_scaling_vector(u_max, dim, "u_max")
    u_mean_arr = _as_scaling_vector(u_mean, dim, "u_mean")
    u_gain_arr = _as_scaling_vector(u_gain, dim, "u_gain")
    _validate_scaler_values(
        u_mean_arr,
        u_gain_arr,
        mean_name="u_mean",
        gain_name="u_gain",
    )
    if not np.all(np.isfinite(u_min_arr)) or not np.all(np.isfinite(u_max_arr)):
        raise ValueError("u_min and u_max must contain only finite values")
    if np.any(u_min_arr >= u_max_arr):
        raise ValueError("u_min must be strictly less than u_max in every dimension")
    return (
        (u_min_arr - u_mean_arr) * u_gain_arr,
        (u_max_arr - u_mean_arr) * u_gain_arr,
    )
...
def _prepare_scaling_info(
    ...
    *,
    ...
    if not use_scaling:
        if y_mean is not None and y_gain is not None:
            y_mean_check = _as_scaling_vector(y_mean, int(y_dim), "y_mean")
            y_gain_check = _as_scaling_vector(y_gain, int(y_dim), "y_gain")
            _validate_scaler_values(
                y_mean_check,
                y_gain_check,
                mean_name="y_mean",
            ...
            if not (
                np.array_equal(y_mean_check, np.zeros(int(y_dim)))
                and np.array_equal(y_gain_check, np.ones(int(y_dim)))
            ):
                raise ValueError("Nonidentity y_mean/y_gain cannot be used when use_scaling=False")
        if u_mean is not None and u_gain is not None:
            u_mean_check = _as_scaling_vector(u_mean, int(u_dim), "u_mean")
            u_gain_check = _as_scaling_vector(u_gain, int(u_dim), "u_gain")
            _validate_scaler_values(
                u_mean_check,
                u_gain_check,
                mean_name="u_mean",
            ...
            if not (
                np.array_equal(u_mean_check, np.zeros(int(u_dim)))
                and np.array_equal(u_gain_check, np.ones(int(u_dim)))
            ):
                raise ValueError("Nonidentity u_mean/u_gain cannot be used when use_scaling=False")
        y_mean, y_gain = 0.0, 1.0
        u_mean, u_gain = 0.0, 1.0
    else:
        if y_mean is None and y_gain is None:
            raise ValueError("use_scaling=True requires explicit y_mean/y_gain values")
        if u_mean is None and u_gain is None:
            raise ValueError("use_scaling=True requires explicit u_mean/u_gain values")
...
def _format_output_feedback_problem(
    ...
    *,
    ...
    y_mean, y_gain:
        ...
        part of the internally formed YYref input. Explicit statistics are
        required when use_scaling=True.
    X_hat_data:
