...
def _normalize_noise_model(noise_model: str) -> str:
    ...
    raise ValueError(f"Unknown measurement-noise model '{noise_model}'")


def _sample_measurement_noise_jax(
    key: Any,
    shape: Any,
    std: Any,
    *,
    ...
    """Sample JAX measurement noise using the configured model."""
    std = jnp.asarray(std)
    model = _normalize_noise_model(noise_model)
    if model == "gaussian":
        return std * jax.random.normal(key, shape=shape)

    bound_arr = 2.0 * jnp.abs(std) if bound is None else jnp.asarray(bound)
    bound_arr = jnp.abs(bound_arr)
    std_abs = jnp.abs(std)
    active = (std_abs > 0.0) & (bound_arr > 0.0)
    safe_std = jnp.where(std_abs > 0.0, std_abs, 1.0)
    limit = bound_arr / safe_std
    noise = std * jax.random.truncated_normal(key, -limit, limit, shape=shape)
    return jnp.where(active, noise, jnp.zeros(shape))


def _sample_measurement_noise_np(
    ...
    *,
    ...
    """Sample NumPy measurement noise using the configured model."""
    std_arr = np.asarray(std, dtype=float)
    model = _normalize_noise_model(noise_model)
    if model == "gaussian":
        return std_arr * rng.normal(size=shape)

    bound_arr = 2.0 * np.abs(std_arr) if bound is None else np.asarray(bound, dtype=float)
    bound_arr = np.abs(bound_arr)
    noise = std_arr * rng.normal(size=shape)
    active = (np.abs(std_arr) > 0.0) & (bound_arr > 0.0)
    if not np.any(active):
        return np.zeros(shape, dtype=float)
    mask = np.abs(noise) > bound_arr
    while np.any(mask):
        noise[mask] = (std_arr * rng.normal(size=shape))[mask]
        mask = np.abs(noise) > bound_arr
    return noise
...
def solve_surrogate_equilibrium(
    ...
    *,
    ...
    def predicted_u(z):
        ...
        return _unscale_array(u_scaled, u_mean, u_gain)
...
_INITIALIZATION_BATCH_RUNNER_CACHE: Dict[Any, Any] = {}


def clear_initialization_batch_runner_cache() -> None:
    """Release cached JIT initialization runners after a large streamed stage."""

    _INITIALIZATION_BATCH_RUNNER_CACHE.clear()
...
def simulate_initialization_trajectories_fast(
    ...
    *,
    ...
    meas_noise_bound: Any = None,
    process_noise_std: Any = 0.0,
    rng_key: Any,
    trajectory_rng_keys: Any = None,
    ...
    cache_key = (
        ...
        _callable_key(mpc_init_fcn),
        _callable_key(ekf_init_meas_fcn),
        _callable_key(ekf_init_time_fcn),
        init_steps,
        str(meas_noise_model),
        _cache_value(meas_noise_std),
        None if meas_noise_bound is None else _cache_value(meas_noise_bound),
        _cache_value(process_noise_std),
        bool(clip_u),
        None if u_min is None else _cache_value(u_min),
        None if u_max is None else _cache_value(u_max),
        str(jax.tree_util.tree_structure(mpc_init_state)),
    )
    batched = _INITIALIZATION_BATCH_RUNNER_CACHE.get(cache_key)
    if batched is None:
        def single_trajectory(x_true, x_hat, P, x_ref_seq, u_ref_seq, key, controller_state):
            ...
            def step(carry, inputs):
                ...
                y_meas = y_true + _sample_measurement_noise_jax(
                    keys_k[0],
                    y_true.shape,
                    meas_noise_std,
                    noise_model=meas_noise_model,
                    bound=meas_noise_bound,
                )
        ...
        _INITIALIZATION_BATCH_RUNNER_CACHE[cache_key] = batched
    if trajectory_rng_keys is None:
        keys = jax.random.split(jnp.asarray(rng_key), x_minus_M.shape[0])
    else:
        keys = jnp.asarray(trajectory_rng_keys)
        expected_key_shape = (num_trajs, 2)
        if keys.shape != expected_key_shape:
            raise ValueError(
                f"trajectory_rng_keys must have shape {expected_key_shape}, got {keys.shape}"
            )
    ...
    if use_initialization:
        ...
        try:
            init_result = simulate_initialization_trajectories_fast(
                ...
                meas_noise_model=meas_noise_model,
                meas_noise_bound=meas_noise_bound,
...
def surrogate_control(
    ...
    u = _unscale_array(u_scaled, scaling_info["u_mean"], scaling_info["u_gain"])
...
def simulate_surrogate_closed_loop(
    ...
    *,
    ...
    measurement_noise_sequence: Optional[Any] = None,
) -> Dict[str, np.ndarray]:
    ...
    noise_bound = None if measurement_noise_bound is None else float(measurement_noise_bound)
    ...
    noise_sequence = None
    if measurement_noise_sequence is not None:
        noise_sequence = _as_reference_trajectory(measurement_noise_sequence)
        if noise_sequence.shape[0] != reference_seq.shape[0]:
            raise ValueError(
                "measurement_noise_sequence must have the same number of steps "
                f"as reference_trajectory: {noise_sequence.shape[0]} != {reference_seq.shape[0]}"
            )
        if noise_sequence.shape[1] != y0.size:
            raise ValueError(
                "measurement_noise_sequence output dimension must match output_fcn: "
                f"{noise_sequence.shape[1]} != {y0.size}"
            )
    ...
    for step, ref in enumerate(reference_seq):
        ...
        U.append(u.copy())
        Y_true.append(y_true.copy())
        Y_meas.append(y_meas.copy())
